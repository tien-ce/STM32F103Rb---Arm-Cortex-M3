...
void timer2_mHz_init(unsigned int usTim1Timerout50us)
{
	...
	RCC->APB1ENR |= TIMER2_CLOCK_EN;
	/*Set the prescaler value*/
	TIM2->PSC	=	400 - 1;	// 8Mhz / 800 = 20 000 Hz ~ 1 tick = 50us
	/*Set the auto-load value*/
	TIM2->ARR	=	usTim1Timerout50us - 1;	// (n*50) us / 1 tick
	/*Clear the counter*/
	TIM2->CNT	=	0;
	/* Clear old interrupt flag*/
	TIM2->SR &= ~(TIMx_SR_UIF);
...
{
	/* Enable TIME2 line in NVIC */
	NVIC_EnableIRQ(TIM2_IRQn);
	/* Enable create interupt when update event occur */
...
void time3_pa6_input_capture (void)
{
	/**** Config intput capture pin ****/
	// Enable clock to GPIOA
	RCC->APB2ENR |= GPIOA_EN;
}
